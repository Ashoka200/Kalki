...
| 🧭 **On-device understanding (no key)** | A ~34 MB sentence-embedding model (bge-small-en-v1.5 via Transformers.js, WebAssembly — runs in iPhone Safari) recognises what a message *means* and routes it to the right skill, so paraphrase, slang and typos the regex rules miss still work — with **no API key and nothing leaving the phone**. The accept threshold self-calibrates against must-not-route examples. Downloaded once, cached, works offline. Toggle in Settings |
| 🧠 **Kalki Brain — on-device LLM (opt-in, no key)** | Settings → *Download Kalki Brain* runs a small instruct model in the browser with WebLLM/WebGPU: Qwen3 1.7B (~2 GB) on capable Android/desktop, Llama 3.2 1B (~0.9 GB) on constrained devices incl. iPhone (experimental — Safari caps a tab near 1.5–2 GB). It extracts details from messy phrasing with grammar-constrained JSON and gives short **labelled** local answers. Strict device gating; unloads when hidden |
| 🌐 **Free web answers (opt-in, no key)** | Off by default. For questions nothing local can answer, legitimate anonymous endpoints (Pollinations, OVHcloud AI Endpoints) reply — rate-limited, clearly labelled *unverified*, never depended on: when busy, Kalki offers a web search instead |
