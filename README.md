...
| 🧭 **On-device understanding (no key)** | A ~23 MB sentence-embedding model (all-MiniLM-L6-v2 via Transformers.js, WebAssembly — runs in iPhone Safari) recognises what a message *means* and routes it to the right skill, so paraphrase, slang and typos the regex rules miss still work — with **no API key and nothing leaving the phone**. Downloaded once, cached, works offline. Toggle in Settings |
